...
`<ApiKeys />` supports creating, renaming, enabling, disabling, and deleting keys. Users can edit permissions, metadata, quotas, refill rules, and rate limits. The list shows request usage and the last request time, with sorting and pagination controls.
...
The server must define every listed `configId`. Use `useUpdateApiKey` for a custom edit surface.
