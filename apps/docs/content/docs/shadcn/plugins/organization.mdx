...
The contents of the `settings` tab — `<OrganizationProfile />`, any plugin-contributed `organizationCards` (e.g. `<OrganizationApiKeys />` from the [api-key plugin](/docs/shadcn/plugins/api-key)), then `<OrganizationDangerZone />`.
