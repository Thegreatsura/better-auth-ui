...
For SSR routes, check the session in `beforeLoad` so unauthenticated users are redirected before any component renders — no flash of unauthenticated UI. Use `createIsomorphicFn` to run `ensureSessionServer` on the server (direct `auth.api` call, no HTTP hop) and `ensureSession` on the client (goes through `authClient`). Both share the same TanStack Query cache under `authQueryKeys.session`, so the session is hydrated from SSR and reused by any child `useSession` call.
