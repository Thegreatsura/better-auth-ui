...
Set `copyable: true` to add a copy button to the input. The button copies the current value, including an edited value.

Use this option with `readOnly: true` for fields such as `id`:
...
Provide a `validate` callback to check a value before submission. If validation fails, throw an `Error`.

The interface shows the error message in a toast:
