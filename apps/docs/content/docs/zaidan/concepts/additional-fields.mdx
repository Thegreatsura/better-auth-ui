...
Install the composed auth payload and the profile payload that render additional fields:

```npm
npx zaidan add https://better-auth-ui.com/r/solid/auth.json https://better-auth-ui.com/r/solid/user-profile.json
```
...
The composed auth/profile payloads include the additional field renderer when those forms need it. If you're building a smaller custom install, you can install the renderer by itself:

```npm
npx zaidan add https://better-auth-ui.com/r/solid/additional-field.json
```
...
Override the visual rendering with `inputType`. The current Zaidan renderer favors semantic native controls instead of inventing shadcn-only primitives that the Solid payload does not ship yet.
...
The shared config supports `inputType: "slider"`, but the current Zaidan payload does not ship a Solid slider primitive. The copied renderer falls back to a native number control for number fields. Replace that branch in `src/components/auth/additional-field.tsx` when your app has a Solid slider component.
...
Both accept an `options` array of `{ label, value }` objects. Zaidan currently renders both as a native `<select>` so the copied payload stays dependency-light.
