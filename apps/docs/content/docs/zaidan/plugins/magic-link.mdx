...
import { createAuthClient } from "@better-auth-ui/solid" // [!code highlight]
...
After a magic-link request succeeds, the form stores the submitted email in session storage and navigates to this confirmation view. It redirects back to the magic-link form when that state is missing and shows an email-provider shortcut when one is available.
...
<auto-type-table path="../../../../../../examples/start-solid-zaidan-example/src/components/auth/magic-link-sent.tsx" name="MagicLinkSentProps" />
