...
Run the Zaidan CLI to install every organization component and the `organizationPlugin()` factory into your project:
...
```tsx file=<rootDir>/../../examples/start-solid-zaidan-example/src/components/auth/organization/organization-switcher.tsx
```
...
```tsx file=<rootDir>/../../examples/start-solid-zaidan-example/src/components/auth/organization/organization.tsx
```
...
```tsx file=<rootDir>/../../examples/start-solid-zaidan-example/src/components/auth/organization/organization-settings.tsx
```
...
Danger-zone section for irreversible organization actions. The Solid/Zaidan implementation includes leave organization and delete organization actions.

### `<OrganizationPeople />`

The contents of the `people` tab. It renders the invite-member dialog, members table, and invitations table.
...
Members table with grouped search, role filtering, sortable table headers, role editing, remove-member action, and current-user leave action.
...
Invitations table with grouped search, role/status filtering, sortable table headers, and cancel action for pending invitations.
...
The settings tab panel mounted at `/settings/organizations`. It renders organizations the user belongs to plus pending invitations addressed to the user.
...
```tsx file=<rootDir>/../../examples/start-solid-zaidan-example/src/components/auth/organization/organizations-settings.tsx
```
...
List of organizations for the signed-in user. Each row links to the slug-aware organization route.

### `<UserInvitations />`

Pending invitations addressed to the signed-in user, with actions to accept or reject invitations.

### `<CreateOrganizationDialog />`

Dialog used by `<OrganizationSwitcher />` and `<Organizations />` to create a new organization.

### `<InviteMemberDialog />`

Dialog used from `<OrganizationPeople />` to invite a member by email and role.

### `<DeleteOrganizationDialog />`

Confirmation dialog used by the organization danger zone before deleting an organization.

## Deferred UI parity

Solid/Zaidan implements the organization runtime, slug-aware routing, settings/profile/delete/leave, user invitation accept/reject, organization switcher, People tables with search/filter/sort, member role edit/remove/leave, invite member, and cancel pending invitation flows.

Advanced invitation status actions and larger copied UI details remain deferred. The Solid package APIs for those flows exist, and organization-scoped API key cards are included through the API Key plugin, but remaining copied organization UI polish stays follow-up work to keep PR #393 reviewable.
