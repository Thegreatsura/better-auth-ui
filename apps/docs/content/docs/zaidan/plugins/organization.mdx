...
- `src/components/auth/organization/delete-organization.tsx` — danger-zone row
...
Use `/organization/$slug/$path` and validate the path segment against `organizationPlugin().viewPaths.organization`.
...
Danger-zone section for irreversible organization actions. The Solid/Zaidan implementation includes delete organization support; leave organization is exposed from the People member row for the current user.
...
Solid/Zaidan implements the organization runtime, slug-aware routing, settings/profile/delete, user invitation accept/reject, organization switcher, People tables with search/filter/sort, member role edit/remove/leave, invite member, and cancel pending invitation flows.
