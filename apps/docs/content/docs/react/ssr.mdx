...
export const Route = createFileRoute("/")({
  loader: ({ context: { queryClient } }) => {
    void prefetchSession(authClient, queryClient)
  }
})
```

The same pattern works for every settings read — `prefetchListAccounts`, `prefetchListSessions`, `prefetchListDeviceSessions`, `prefetchListUserPasskeys`, and `prefetchAccountInfo` all live in `@better-auth-ui/react` next to their options factories.
...
export const ensureServerSession = (queryClient: QueryClient) =>
  ensureSession(auth, queryClient, { headers: getRequestHeaders() })
```

The server entrypoint mirrors the client API one-to-one: `sessionOptions`, `ensureSession`, `prefetchSession`, `fetchSession`, plus the equivalents for `listAccounts`, `accountInfo`, `listSessions`, `listDeviceSessions`, and `listUserPasskeys`. Cache keys are identical between the two, so data prefetched on the server hydrates cleanly into the client-side `useSession`, `useListSessions`, etc.
