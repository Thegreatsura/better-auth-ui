...
`<ApiKeys />` supports the complete key lifecycle. Users can create, rename, enable, disable, and delete keys. They can also edit permissions, metadata, quotas, refill rules, and rate limits. The list shows request usage and the last request time, with sorting and pagination controls.
...
The server must define every listed `configId`. Use `useUpdateApiKey` when you build a custom edit surface.
