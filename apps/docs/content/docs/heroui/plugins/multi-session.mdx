...
import { multiSessionPlugin } from "@better-auth-ui/heroui/plugins" // [!code highlight]
...
The `<ManageAccounts />` card is automatically rendered in account settings when the plugin is registered. It can also be used standalone:
...
See the `<ManageAccounts />` component in `@better-auth-ui/heroui/plugins` for props and customization.
