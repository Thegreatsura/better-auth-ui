...
- An `organizationCards` plugin slot rendered inside `<OrganizationSettings />` so other plugins (e.g. [api-key](/docs/heroui/plugins/api-key)) can attach org-scoped cards
...
Set `slugPrefix` when the visible route should include a marker, such as `/organization/@acme/settings`. Keep `slug` itself unprefixed. For example, a TanStack route named `/organization/@{$slug}/$path` captures `acme`, while `organizationPlugin({ slug, slugPrefix: "@" })` renders the prefix in fields and labels and includes it in generated links.
...
The contents of the `settings` tab — `<OrganizationProfile />`, any plugin-contributed `organizationCards` (e.g. `<OrganizationApiKeys />` from the [api-key plugin](/docs/heroui/plugins/api-key)), then `<OrganizationDangerZone />`. Drop it into a custom layout if you don't want the tabbed shell.
